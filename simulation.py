...
class Simulation:
    ...
    def save_pytorch(
        ...
        *,
        ...
        epoch=None,
        prefix="",
        default_body="checkpoint",
    ):
        outdir = join(self._outdir, subdir)
        setup_outdir(outdir)

        if epoch is None:
            datafile = join(outdir, f"{prefix}{default_body}.pt")
        else:
            datafile = join(outdir, f"{prefix}{default_body}_epoch{epoch}.pt")
        ...
        if mode == "pkl":
            ...
            with h5py.File(datafile, "w" if overwrite else "w-") as f:
                for k, v in data.items():
                    ...
                    ), "data values must be of type np.ndarray"
    ...
    def save_data_hdf5(self, data, title="", *, subdir=None, overwrite=False):
        return self.save_data(
            data, title, subdir=subdir, overwrite=overwrite, mode="hdf5"
        )
    ...
    def __save_code(self):
        ...
        extensions = ["py", "sh"]
        files = [
            x for ext in extensions for x in glob.glob(join(self._codedir, "*." + ext))
        ]
    ...
    def received_sigint(self):
        if not self._catch_sigint:
            raise ValueError(
                "attempting to read out sigint status, but "
